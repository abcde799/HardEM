'''
This is a script for the purpose of hypothesis testing the effect of inclusion of covariates linked to the lifetime component in the model, which appears in the survivor
function. The fit functions are similar to those appearing in 'survival_func.py' but have the prefix 'nc' to denote no covariates. 
'''
...
import pandas as pd
from survival_func import mod_prob_density, mod_overall_survival, survival_fit   # Custom modules here

####################################################################################################################################################
####################################################################################################################################################
#FUNCTIONS FOR FITTING AND HELPER FUNCTIONS 
...
def nc_survival_fit_weights(censored_inputs, noncensored_inputs, nc, maxiter):
    ...
    def training_loss(flatparam):
        ...
        known_loss = np.log(
            np.array(mod_prob_density(noncensored_inputs, param))
        )  # noncensored loss term
        unknown_loss = np.log(
            np.array(mod_overall_survival(censored_inputs, param))
        )  # censored loss term
...
def nc_survival_fit(censored_inputs, noncensored_inputs, nc, maxiter):
    ...
    while result is None:
        ...
        try:
            ...
            result = nc_survival_fit_weights(
                censored_inputs, noncensored_inputs, nc, 100
            )

        except:

            print('Error with autograd trying again.')
...
####################################################################################################################################################
####################################################################################################################################################
#PREPARE MELANOMA DATA AND FIT 100 TIMES 
...
if __name__ == "__main__":
    ...
    def prepare_data(mel):
        ...
        mel_covariates = (
            mel_covariates - mel_covariates.mean()
        ) / mel_covariates.std()  # standardize col. wise.
        ...
        noncensored_inputs = make_inputs(
            mel_covariates, 1, columns
        )  # extract noncensored
        ...
        final_cov = final[
            [
                ...
                if col not in ["time", "predicted_prob_cured"]
            ]
        ]
        ...
        final_time_prob_cens = (final[final.status == 0])[
            ["time", "predicted_prob_cured"]
        ]

        final_time_prob_noncens = (final[final.status == 1])[
            ["time", "predicted_prob_cured"]
        ]
    ...
    for i in range(100):
        ...
        arr.append(
            survival_fit(
                censored_inputs, noncensored_inputs, 0, 100, log_likelihood=True
            )
        )
    ...
    df.to_csv("hyp. test. after 100 runs.csv", index=False) # Persist output to csv 
