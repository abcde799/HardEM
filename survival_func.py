...
def susc_survival(time, covariate_vector, scaling, shape, gamma):
    ...
    assert (
        np.exp(arg) >= 0
    ), "arg is {}. Output of 'susc_survival' is not a nonnegative number".format(arg)
...
def overall_survival(time, prob, covariate_vector, scaling, shape, gamma):
    ...
    out = prob + (1 - prob) * susc_survival(
        time, covariate_vector, scaling, shape, gamma
    )
...
def prob_density(time, prob, covariate_vector, scaling, shape, gamma):
    ...
    def time_slice(time_param):
        return overall_survival(
            time_param, prob, covariate_vector, scaling, shape, gamma
        )
...
def mod_prob_density(Array, param):
    ...
    out = [
        prob_density(arr[0], arr[1], arr[2], param[0], param[1], param[2])
        for arr in Array
    ]
...
def mod_overall_survival(Array, param):
    ...
    out = [
        overall_survival(arr[0], arr[1], arr[2], param[0], param[1], param[2])
        for arr in Array
    ]
