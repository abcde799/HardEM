...
def add_nscar_censoring_indicator(df):
    ...
    df["nscar_censoring_indicator"] = df.apply(
        lambda x: aux(x.cure_label, x.nscar_cens_label), axis=1
    )
...
def create_df(
    covariates, dist, n_patients, test_model_weights, prob_cens_given_not_cured
):
