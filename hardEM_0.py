...
def HEM_fit(censored_inputs, noncensored_inputs, C, maxiter, initialize):
    ...
    def training_loss(param):
        ...
        weights, unknownlabels = (
            param[0 : len(censored_inputs[0])],
            param[len(censored_inputs[0]) :],
        )
        ...
        unknown_loss = (
            np.log(prob(weights, censored_inputs)) * (1 - unknownlabels)
            + np.log(1 - prob(weights, censored_inputs)) * unknownlabels
        )
    ...
    if initialize == "censoring_rate":
        ...
        p = (
            1 - n_cens / n_rows
        )  # probability of getting cured (lanubel 0!) is the censored rate.
        ...
        fit = {
            ...
            "unknown_cure_labels": unknown_cure_labels,
        }
        ...
        fit = {
            "model_weights": optimal_model_weights,
            "unknown_cure_labels": optimal_model_labels,
            "minvalue": minim,
            "maxvalue": maxim,
        }
        ...
        raise ValueError(
            "Need initialize parameter to be chosen as either 'use_clustering' or 'censoring_rate', or 'use_random' "
        )

####################################################################################################################################################
####################################################################################################################################################
#PREDICT AND IMPUTE FUNCTIONS 
...
def HEM_predictions(model_weights, df, covariates):
    ...
    predictions_float = 1 - sigmoid(
        model_weights[0] * intercept + np.dot(table, nonintercept_weights)
    )  # This is prob of not cured!
